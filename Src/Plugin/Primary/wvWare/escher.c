...
wv0x08 (Blip * blip, S32 spid, wvParseStruct * ps)
{
    ...
    if (ps->escher == NULL)
    {
        ps->escher = (escherstruct *) wvMalloc (sizeof(escherstruct));
        wvGetEscher (ps->escher, ps->fib.fcDggInfo, ps->fib.lcbDggInfo, ps->tablefd,
		    ps->mainfd);
    }

    for (i = 0; i < ps->escher->dgcontainer.no_spgrcontainer; i++)
      {
	  answer = wvFindSPID (&(ps->escher->dgcontainer.spgrcontainer[i]), spid);
	  if (answer)
	      break;
      }
		...
		if (answer->fopte[i].pid == 260)
		  {
		      ...
		      wvTrace (
			       ("no blips is %d\n",
				ps->escher->dggcontainer.bstorecontainer.no_fbse));
		      wvTrace (
			       ("type is %d (number is %d\n",
				ps->escher->dggcontainer.bstorecontainer.blip[ps->escher->
								       dggcontainer.
								       bstorecontainer.
								       no_fbse -
								       1].type,
			  ...
			  ps->escher->dggcontainer.bstorecontainer.no_fbse)
			{
			    wvTrace (("Copied Blip\n"));
			    wvCopyBlip (blip,
					&(ps->escher->dggcontainer.bstorecontainer.
    ...
    while (count < msofbh->cbLength)
	  ...
	  switch (amsofbh.fbt)
	    ...
	    case msofbtClientAnchor:
                /* The format of the field is host-defined, so there is no way we
                   could usefully process it. */
                count += wvEatmsofbt (&amsofbh, fd);
                break;
...
wvGetMSOFBH (MSOFBH * amsofbh, wvStream * fd)
{
    ...
    if ( amsofbh->fbt < 0xF000 )
    {
        wvTrace (("Broken MSOFBH\n"));
    }
    return (8);
}
